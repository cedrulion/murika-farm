...
exports.updateUserById = async (req, res) => {
  try {
      ...
      if (req.user.role !== 'admin') {
          return res.status(403).json({ message: 'Forbidden: Only administrators can update other user profiles.' });
      }
      ...
      res.status(200).json({ message: 'User updated successfully', updatedUser: userWithoutPassword });
